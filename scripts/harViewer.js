        ...
        _validate: function (a, b, c) {
            function f(a, b, f, g) {
                if (typeof b == "object") {
                    ...
                    for (var h in b)
                        if (b.hasOwnProperty(h) && (h.charAt(0) != "_" || h.charAt(1) != "_")) {
                            var i = a[h],
                                j = b[h];
                            e(i, j, f, h)
                        }
                        ...
                        if (a instanceof Array) {
                            if (b.items)
                                if (b.items instanceof Array)
                                    for (h = 0, i = a.length; h < i; h++) d.concat(e(a[h], b.items[h], g, h));
                                else
                                    for (h = 0, i = a.length; h < i; h++) d.concat(e(a[h], b.items, g, h));
    ...
    var b = {
        resolveJson: function (b, c) {
            function n(o, p, q, r, s) {
                ...
                if (t) {
                    for (A in w)
                        if (!w.__isDirty && w.hasOwnProperty(A) && !o.hasOwnProperty(A) && A != "__id" && A != "__clientId" && !(w instanceof Array && isNaN(A))) {
...
function ($) {
    ...
    h.prototype = {
        ...
        run: function (b, c) {
            ...
            var e = a.getURLParameters("path"),
                ...
                g = a.getURLParameters("inputUrl") || getAnchorParameters("inputUrl"),
                h = [];
    ...
    with(Domplate) {
        var Controller = {
            ...
            removeController: function () {
                for (var a = 0, b; b = arguments[a]; a++)
                    for (var c = 0, d; d = this.controllers[c]; c++) b[0] == d[0] && b[1] == d[1] && b[2] == d[3] && Lib.removeEventListener.apply(this, d)
            },
    ...
    with(Domplate) {
        ...
        Toolbar.prototype = {
            ...
            getButton: function (a) {
                for (var b = 0; b < this.buttons.length; b++)
                    if (this.buttons[b].id == a) return this.buttons[b]
            },
            render: function (a) {
    ...
    with(Domplate) {
        ...
        Timeline.prototype = domplate({
            ...
            recalcLayout: function () {
                ...
                if (a != this.maxElapsedTime)
                    for (var c = 0; c < b.length; c++) b[c].style.height = this.getHeight(b[c].page) + "px"
            },
            updateDesc: function () {
                if (this.isVisible())
    ...
    a.SyntaxHighlighter = a.sh, a.sh.Toolbar.Commands = {
        ...
        function e(b) {
            ...
            if (g > 0)
                for (var h = 0; h < d.length; h++) d[h] = d[h].substr(g);
            return d.join("\n")
        }
                ...
                if (a) {
                    ...
                    for (var c = 0; c < g.length; c++) {
                        ...
                        for (var j = 0; j < b.length; j++) {
                            ...
                            l += m;
                            if (!i || l < i.startTime)
                                if (c == 0 || l >= h.startTime) h.startTime > l && (h.startTime = l), h.endTime < l && (h.endTime = l), h.pageTimings.push({
    ...
    return {
        ...
        embedSWF: function (c, d, e, f, g, h, i, j, k) {
            if (t.w3cdom && c && d && e && f && g) {
                ...
                if (L(g)) {
                    ...
                    var l = {};
                    if (k && typeof k === b)
                        for (var m in k) k[m] != Object.prototype[m] && (l[m] = k[m]);
                    l.data = c, l.width = e, l.height = f;
                    var n = {};
                    if (j && typeof j === b)
                        for (var o in j) j[o] != Object.prototype[o] && (n[o] = j[o]);
                    if (i && typeof i === b)
                        for (var p in i) i[p] != Object.prototype[p] && (typeof n.flashvars != a ? n.flashvars += "&" + p + "=" + i[p] : n.flashvars = p + "=" + i[p]);
                    x(function () {
        ...
        getQueryParamValue: function (a) {
            ...
            if (b) {
                ...
                for (var d = 0; d < c.length; d++)
                    if (c[d].substring(0, c[d].indexOf("=")) == a) return O(c[d].substring(c[d].indexOf("=") + 1))
            }
            return ""
        },
...
function () {
    ...
    with(Domplate) {
        var Search = {}, caseSensitiveOption = "searchCaseSensitive";
        Search.Box = domplate({
            tag: SPAN({
                "class": "searchBox"
            }, SPAN({
